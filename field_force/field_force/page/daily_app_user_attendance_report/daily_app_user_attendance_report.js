frappe.pages['daily-app-user-attendance-report'].on_page_load = (wrapper) => {
...
class DailyAppUserAttendanceReport {
    ...
    fetch_and_render = () => {
        let {from_date, to_date, sales_person} = this.form.get_values();
        if (!from_date) {
        ...
        `);
        frappe.call('field_force.field_force.page.daily_app_user_attendance_report.daily_app_user_attendance_report.get_user_attendance_data', {
            filters: {
                from_date: from_date,
                to_date: to_date,
    ...
    export_excel = () => {
        let {from_date, to_date, sales_person} = this.form.get_values();
        let url = `/api/method/field_force.field_force.page.daily_app_user_attendance_report.daily_app_user_attendance_report.export_file`;
        url += `?from_date=${from_date||''}&to_date=${to_date||''}&sales_person=${sales_person||''}`;
