...
def get_columns(filters):
    ...
    return [
        ...
        {"label": _("UOM"), "fieldtype": "Link", "fieldname": "uom", "options": "UOM", "width": 70},
        {
            "label": _("Rate"),
            "fieldname": "rate",
            "fieldtype": "Currency",
...
def get_sales_order_details(company_list, filters):
    doctype = filters.get('doctype')
    db_so = frappe.qb.DocType(doctype)
    db_so_item = frappe.qb.DocType(f"{doctype} Item")
...
def get_chart_data(data):
    ...
    for row in data:
        item_key = f"{row.get('item_code')} - {row.get('product_id')}<br>{row.get('item_name')}"
