...
def get_data(filters, conditions):
    ...
    if conditions.get("trans") in ["Sales Order", "Requisition", "Purchase Order"]:
        ...
        for d in range(len(data1)):
            ...
            row = frappe.db.sql(
                """select DISTINCT(%s) from `tab%s` t1, `tab%s Item` t2 %s
                        ...
                        and t1.docstatus = 1 and %s = %s %s %s order by t2.amount
                    """
                % (
                    sel_col,
                    conditions["trans"],
            ...
            for i in range(len(row)):
                ...
                row1 = frappe.db.sql(
                    """ select %s , %s from `tab%s` t1, `tab%s Item` t2 %s
                            where t2.parent = t1.name and t1.company = %s and %s between %s and %s
                            and t1.docstatus = 1 and %s = %s and %s = %s %s %s order by t2.amount
                        """
                    % (
                        sel_col,
...
def based_wise_columns_query(based_on, trans):
    ...
    if based_on == "Brand":
        based_on_details["based_on_cols"] = ["Brand:Link/Brand:120", "Brand:Data:120"]
        based_on_details["based_on_select"] = "t2.brand, t2.brand,"
        ...
        if trans in ["Sales Invoice", "Delivery Note", "Sales Order", "Requisition"]:
