(() => {
  ...
  // ../field_force/field_force/public/js/get_item/multi_select_dialog.js
        ...
        if (column === "customer" || column === "expected_delivery_date" || column === "name") {
          if (column === "customer") {
            contents += `<div class="list-item__content ellipsis">
                        ${head ? `<span class="ellipsis text-muted" title="${__(frappe.model.unscrub(column))}">${__(frappe.model.unscrub(column))}</span>` : `<a href="${"/app/customer/" + result[column] || ""}" class="list-id ellipsis" title="${__(result[column] || "")}">
                                    ${__(result[column] || "")}</a>`}
                    </div>`;
          } else if (column === "expected_delivery_date" && result[column]) {
            contents += `<div class="list-item__content ellipsis">
                        ${head ? `<span class="ellipsis text-muted" title="${__(frappe.model.unscrub(column))}">${__(frappe.model.unscrub(column))}</span>` : `<span class="ellipsis result-row" title="${__(result[column] || "")}">${__(moment(result[column]).format("DD-MM-YYYY") || "")}</span>`}
                    </div>`;
          } else {
            contents += `<div class="list-item__content ellipsis">
...
//# sourceMappingURL=field_force.bundle.K4UOQELW.js.map
